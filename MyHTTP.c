...
int timecheck(char *file_path,char *token)
{
	...
	if (stat(file_path, &file_info) == 0) 
	{
		...
		{
			char buf[50];
			strftime(buf,50,"%a, %d %b %Y %T GMT",&timesent);
			//printf("%s\n",buf);
			time_t t1=mktime(&timesent);
			time_t t2=mktime(last_modified);
			//printf("%s\n",t1);
			
			int k=difftime(t1,t2);
			if(k<=0) 
			{
...
int headercheck(char **headers,int n,int gorp,char *resp,long long int* len)
{
     ...
     for(int i=1;i<n;i++)
	 {
		...
		else if(strcmp(token,"If-Modified-Since:")==0)
		{
...
int main()
{
	...
	while (1) {
		...
		if (fork() == 0) {
			...
			{
				FILE *fp1;
				fp1 = fopen("AccessLog.txt", "a");  // "a" mode opens the file for appending
				

				fprintf(fp1,"%d-%d-%d : ", year, month, day);
				fprintf(fp1,"%d:%d:%d : ", hour, minute, second);
				fprintf(fp1,"%s : ",cli_ip);
				fprintf(fp1,"%d : ",port_num);
				fprintf(fp1,"GET : %s\n",file_path);
